...
class VG_Attribution(Dataset):
    ...
    def evaluate_scores_bootstrap(self, pred_idx, scores):
        ...
        all_attributes = np.array(self.all_attributes)
        all_attributes = all_attributes[pred_idx]

        result_records = []
        # Log the accuracy of all attributes
        for attribute in np.unique(all_attributes):
            ...
            result_records.append({
                "Attribute": attribute,
                "Accuracy": correct_mask[attribute_mask].mean(),
                "Count": attribute_mask.sum(),
                "Dataset": "Visual Genome Attribution"
            })
        return result_records
...
class COCO_Order(Dataset):
    ...
    def evaluate_scores_bootstrap(self, pred_idx, scores):
        ...
        Scores: N x 1 x 2, i.e. first caption is the perturbed one, second is the positive one
        """
        return self.evaluate_scores(scores)
