...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../hycoclip'))
import numpy as np
import pandas as pd
import argparse
import pickle
...
parser.add_argument('--id-data', default='imagenet', type=str, choices=['imagenet', 'cifar10', 'cifar100'])
args = parser.parse_args()

model_from_hycoclip_repo = any(arch in args.model_type for arch in ['meru', 'hycoclip', 'hyclip'])

assert not (model_from_hycoclip_repo and args.checkpoint is None), \
    "Please provide a checkpoint for HyCoCLIP models."
...
if args.model_type == 'clip':
    ...
    net = CLIPZeroshot(classnames=class_names[args.id_data],
                       templates=prompt_templates[args.id_data],
                       backbone=args.arch)
...
elif model_from_hycoclip_repo:
    from openood.networks import HyperbolicCLIPZeroshot
    net = HyperbolicCLIPZeroshot(classnames=class_names[args.id_data],
                       templates=prompt_templates[args.id_data],
