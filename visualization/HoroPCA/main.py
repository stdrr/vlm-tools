...
parser.add_argument('--dataset', type=str, help='which datasets to use', default="smalltree",
                    choices=["smalltree", "phylo-tree", "bio-diseasome", "ca-CSphd", "grit"])
...
parser.add_argument(
    "--downsample", default=10000, type=int, help="downsample the number of nodes for large graphs"
)
...
if __name__ == "__main__":
    ...
    downsample = args.downsample if args.downsample > 0 else None
    ...
    if args.use_sarkar:
        ...
        import pickle
        with open("data/embeddings/grit_poincare.pkl", "rb") as f:
            Z = pickle.load(f)
        groups = list(Z.keys())
        if downsample:
            n_samples = Z[groups[0]].shape[0]
            z = np.vstack([Z[g][:min(n_samples, downsample)] for g in groups])
            logging.info(f"Downsampled to {z.shape[0]} nodes")
        else:
            z = Z
        breakpoint()
        z = torch.from_numpy(z).to(torch.float32).to("cuda:0")
    ...
    if args.model in pca_models.keys():
        ...
        # reduce the full data
        logging.info("Projecting the full dataset")
        Z_proj = {}
        for g in groups:
            zt = torch.from_numpy(Z[g]).to(torch.float32).to(x.device)
            Z_proj[g] = model.map_to_ball(zt).detach().cpu().numpy()
            # Z_centered = poincare.reflect_at_zero(Z, mu_ref)
        
        # save the embeddings
        logging.info("Saving the embeddings")
        with open(f"data/embeddings/{args.dataset}_{args.model}_{args.n_components}d.pkl", "wb") as f:
            pickle.dump(Z_proj, f)
        logging.info(f"Dimensionality reduction with {args.model} completed.")
    else:
        # run hMDS baseline
