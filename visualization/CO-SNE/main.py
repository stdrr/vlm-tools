...
def plot_low_dims(tsne_embeddings, HT_SNE_embeddings, CO_SNE_embedding, colors, learning_rate, learning_rate_for_h_loss, perplexity, early_exaggeration, student_t_gamma):
    ...
    sns.scatterplot(x=tsne_embeddings[:, 0], y=tsne_embeddings[:, 1], ax=ax, color=colors, s=30)
    ax.set_aspect('equal')
    ax.axis('off')
    ...
    sns.scatterplot(x=HT_SNE_embeddings[:,0], y=HT_SNE_embeddings[:,1], ax=ax, color=colors, s=30)
    ax.set_aspect('equal')
    ax.axis('off')
    ...
    circle1 = plt.Circle((0, 0), 1, color='lightgray', fill=True, alpha=0.5)
    plt.gca().add_patch(circle1)

    sns.scatterplot(x=CO_SNE_embedding[:,0], y=CO_SNE_embedding[:,1], ax=ax, color=colors, s=10)
    ax.set_aspect('equal')
    ax.axis('off')
...
if __name__ == "__main__":
    ...
    import pickle
    with open("../HoroPCA/data/embeddings/grit_poincare.pkl", "rb") as f:
        Z = pickle.load(f)
    groups = ["box_text_feats", "text_feats", "box_image_feats", "image_feats"]
    embeddings = np.vstack([Z[g][:100] for g in groups])
    embeddings = torch.tensor(embeddings).float()
    print("embs", embeddings.shape)
    
    n_samples = embeddings.shape[0] // 4
    colors = ['green'] * n_samples + ['orange'] * n_samples + ['blue'] * n_samples + ['red'] * n_samples
